...
PARTSDB = "W:/partsdb.txt"
#"S:/Dropbox/EV/2016 Hardware/partsdb.txt"
#C:/usafirmware_skirmoshw/BOM/partsdb.txt
...
with open(PARTSDB, 'a') as file:
	...
	if mfgPartNo is not None and mfg is not None and desc is not None and package is not None:
		...
		entry = "%s\t%s\tDK\t%s\t%s\t%s\n" % (mfgPartNo, mfg, partNo, desc.strip(), package.strip())
		print entry
		entry.encode('ascii', 'ignore')
		file.write(entry)
